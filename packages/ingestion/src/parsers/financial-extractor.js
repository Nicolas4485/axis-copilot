 ...
 */
function classifyMetric(line) {
    const lower = line.toLowerCase();
    if (/\b(total\s+)?revenue|net\s+revenue|total\s+sales?|net\s+sales?\b/.test(lower))
        return 'revenue';
    if (/\barr\b|\bannual\s+recurring\s+revenue/.test(lower))
        return 'arr';
    if (/\bebitda\b(?!\s+margin)/.test(lower))
        return 'ebitda';
    if (/\bgross\s+profit\b(?!\s+margin)/.test(lower))
        return 'grossProfit';
    if (/\bnet\s+income\b|\bnet\s+earnings?\b/.test(lower))
        return 'netIncome';
    if (/\bgross\s+margin\b/.test(lower))
        return 'grossMargin';
    if (/\bebitda\s+margin\b/.test(lower))
        return 'ebitdaMargin';
    return null;
}
// ─── Year detection ───────────────────────────────────────────────────────────
 ...
 */
function extractYearLabels(line) {
    const years = [];
 ...
 */
export async function extractFinancials(pdfBuffer) {
    ...
    for (const page of financialPages.slice(0, 10)) { // process top 10 financial pages
        ...
        if (page.score >= 3 && rawSnippets.length > 0) {
            ...
            if (snippetLine.length > 30)
                rawSnippets.push(`[p.${pageNum} header] ${snippetLine}`);
        }
    }
    if (rows.length === 0)
        return null;
    ...
    if (detectedYears.length < numYears) {
        ...
        detectedYears = Array.from({ length: numYears }, (_, i) => String(currentYear - (numYears - 1 - i)));
    }
    ...
    let confidence = 'low';
    if (hasRevenue && hasEbitda && hasMultiYear)
        confidence = 'high';
    else if (hasRevenue && hasMultiYear)
        confidence = 'medium';
    return {
        years,
        currency,
 ...
 */
export function formatFinancialsForPrompt(data) {
    ...
    const revRow = data.years.map((y) => y.revenue != null ? `${currencySymbol}${y.revenue}${unitLabel}`.padEnd(8) : 'n/a     ').join('');
    const ebitdaRow = data.years.map((y) => y.ebitda != null ? `${currencySymbol}${y.ebitda}${unitLabel}`.padEnd(8) : 'n/a     ').join('');
    const ebitdaMarginRow = data.years.map((y) => y.ebitdaMargin != null ? `${y.ebitdaMargin}%`.padEnd(8) : 'n/a     ').join('');
    const growthRow = data.years.map((y) => y.revenueGrowth != null ? `${y.revenueGrowth > 0 ? '+' : ''}${y.revenueGrowth}%`.padEnd(8) : '—       ').join('');
