...
export class HybridRetriever {
     ...
     */
    async retrieve(query, userId, clientId, queryEmbedding) {
     ...
     */
    async vectorSearch(query, userId, clientId, queryEmbedding) {
        const allChunks = [];
        ...
        `;
                const params = [vectorStr, userId, VECTOR_SIMILARITY_THRESHOLD];
                if (clientId) {
     ...
     */
    async queryGraphEntity(entityName, relationshipTypes, depth = 2) {
        if (!this.neo4jClient.isAvailable())
            return [];
        try {
            const result = await this.graphOps.findRelated(entityName, depth, relationshipTypes && relationshipTypes.length > 0
                ? relationshipTypes
                : undefined);
            if (!result)
                return [];
            const relationships = result.relationships.map((r) => ({
                ...
                targetType: 'label' in r.targetNode
                    ? r.targetNode.label
                    : 'Unknown',
                properties: Object.fromEntries(Object.entries(r.relationship).filter(([k]) => !['type', 'fromId', 'toId'].includes(k))),
            }));
            return [{
    ...
    async graphTraversal(query) {
        ...
        for (const gq of query.graphQueries) {
            try {
                const result = await this.graphOps.findRelated(gq.entityName, gq.depth || DEFAULT_GRAPH_DEPTH, gq.relationshipTypes.length > 0
                    ? gq.relationshipTypes
                    : undefined);
                if (result) {
                    const relationships = result.relationships.map((r) => ({
                        type: r.relationship.type,
                    ...
                    const readableLines = relationships.map((r) => `${result.node.name} -[${r.type}]-> ${r.targetName} (${r.targetType})`);
                    insights.push({
                        entityName: result.node.name,
                        entityType: 'label' in result.node
                            ? result.node.label
        ...
        for (const entity of query.entityFocus) {
            if (insights.some((i) => i.entityName === entity))
                continue;
            try {
