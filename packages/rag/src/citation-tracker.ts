...
/** Citation metadata stored in Message.metadata.citations */
export interface MessageCitationMetadata {
  citations: Citation[]
  totalSources: number
...
export class CitationTracker {
   ...
   */
  buildMetadata(citations: Citation[]): MessageCitationMetadata {
   ...
   */
  formatInlineCitations(citations: Citation[]): string {
    if (citations.length === 0) return ''

    const lines = citations.map(
      (c, i) => `[${i + 1}] ${c.sourceTitle} (relevance: ${(c.relevanceScore * 100).toFixed(0)}%)`
    )
   ...
   */
  extractReferences(responseText: string): number[] {
    const refs: number[] = []
    const pattern = /\[(\d+)\]/g
  ...
  validateReferences(
    responseText: string,
    citations: Citation[]
  ): { valid: boolean; missingRefs: number[] } {
